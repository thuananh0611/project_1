    ...
    public class FormHD_Nhap
    {
        public void Nhap()
        {
            do
            {
                ...
                hdn.Manv = int.Parse("0" + IO.ReadString(63, 4));
                hdn.Sl = int.Parse("0" + IO.ReadNumber(14, 6));
                hdn.Mancc = int.Parse("0" + IO.ReadString(32, 6));
        ...
        public void SuaHD_Nhap()
        {
            do
            {
                ...
                double vat;
                double thanhtien;
                mhdn = int.Parse("0" + IO.ReadString(15, 4));
                HD_Nhap hdn = hdnhap.LayHD_Nhap(mhdn);
                hdn.Mhdn = int.Parse("0" + IO.ReadNumber(15, 4));
                hdn.Tenhd = IO.ReadString(18, 4);
                hdn.Manv = int.Parse("0" + IO.ReadString(52, 4));
                hdn.Sl = int.Parse("0" + IO.ReadNumber(15, 6));
                hdn.Mancc = int.Parse("0" + IO.ReadString(19, 6));
                hdn.Donvi = IO.ReadString(31, 6);
                hdn.Gianhap = double.Parse("0" + IO.ReadNumber(43, 7));
                hdn.VAT = double.Parse("0" + IO.ReadNumber(50, 7));
                hdn.Thanhtien = double.Parse("0" + IO.ReadNumber(72, 7));

                mhdn = int.Parse("0" + IO.ReadNumber(15, 4));
                if (mhdn != hdn.Mhdn && mhdn != 0) hdn.Mhdn = mhdn;
                tenhd = IO.ReadString(18, 4);
                if (tenhd != hdn.Tenhd && tenhd != null) hdn.Tenhd = tenhd;
                manv = int.Parse("0" + IO.ReadNumber(52, 4));
                if (manv != hdn.Manv && manv != 0) hdn.Manv = manv;
                sl = int.Parse("0" + IO.ReadNumber(15, 6));
                if (sl != hdn.Sl && sl != 0) hdn.Sl = sl;
                mancc = int.Parse("0" + IO.ReadString(19, 6));
                if (mancc != hdn.Mancc && mancc != 0) hdn.Mancc = mancc;
                donvi = IO.ReadString(31, 6);
                if (donvi != hdn.Donvi && donvi != null) hdn.Donvi = donvi;
                gianhap = double.Parse("0" + IO.ReadNumber(43, 7));
                if (gianhap != hdn.Gianhap && gianhap != 0) hdn.Gianhap = gianhap;
                vat = double.Parse("0" + IO.ReadNumber(50, 7));
                if (vat != hdn.VAT && vat != 0) hdn.VAT = vat;
                thanhtien = double.Parse("0" + IO.ReadNumber(72, 7));
